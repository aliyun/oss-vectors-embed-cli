...
echo "🔍 Validating OSS Vectors CLI package for PyPI readiness..."
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    try:
        ...
        # Test import
        result = subprocess.run([python_path, '-c', 'import oss_vectors; print(\"✅ Package imports successfully\")'],
                              capture_output=True, text=True, check=True)
        print(result.stdout.strip())
        
        # Test CLI command
        result = subprocess.run([python_path, '-c', 'from oss_vectors.cli import main; print(\"✅ CLI entry point works\")'],
                              capture_output=True, text=True, check=True)
        print(result.stdout.strip())
